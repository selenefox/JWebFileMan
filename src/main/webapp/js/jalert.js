 ...
 */
function jAlert(_context,_title,_callback,_surelang){
    var _w = 300,_h=135;
    ...
    if(undefined==_surelang || ""==_surelang)
        _surelang = "确定";
    result.push('<button type="button" style="'+sm_btn+'" id="okAlert">'+_surelang+'</button>');
    result.push('</div>');
...
function jPrompt(_title,_callback,_inputtype){
    var _w = 300,_h=135;
    var iWidth =  $(window).width();
    var iHeight =  $(window).height();
    var iTop = $(window).scrollTop();
    ...
    if(undefined==_title || ""==_title)
        _title = "提示";
    result.push('<div style="float: left;width: 100%;height: 40px;line-height: 40px;font-weight: bold;/*border-bottom: 1px solid #f1f1f1;*/text-align: center;">'+_title+'</div>');
    if(undefined == _inputtype || ""==_inputtype){
        result.push('<div id="popContext" style="float: left;width: 100%;height: 50px;/*line-height: 50px;*/padding-top:12px;text-align: center;"><input id="jprompt-input-id" style="width: 80%" type="password"/></div>');
    }else{
        result.push('<div id="popContext" style="float: left;width: 100%;height: 50px;/*line-height: 50px;*/padding-top:12px;text-align: center;"><input id="jprompt-input-id" style="width: 80%" type="'+_inputtype+'"/></div>');
    }
    result.push('<div style="float: left;width: 100%;text-align: center;/*border-top: 1px solid #f1f1f1;*/height: 40px;">');
    result.push('<button type="button" style="'+pri_btn+'" id="okAlert">确定</button>');
    result.push('<button type="button" style="'+sm_btn+'margin-left: 20px;" id="cancelAlert">取消</button>');
